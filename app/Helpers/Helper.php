...
class Helper
{
    ...
    static function URLSlug($url_ru, $url_kz, $url_en, $type = "", $id = 0)
    {
        $Check_SEO_st_ru = true;
        $Check_SEO_st_kz = true;
        $Check_SEO_st_en = true;

        $seo_url_slug_ru = str_slug($url_ru, '-');
        $seo_url_slug_kz = str_slug($url_kz, '-');
        ...
        if ($type == "section" && $id > 0) {
            // .. ..  Webmaster Sections
            $check_WebmasterSection = WebmasterSection::where([['seo_url_slug_ru', $seo_url_slug_ru], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_ru], ['id', '!=', $id]])->get();
            if (count($check_WebmasterSection) > 0) {
                $Check_SEO_st_ru = false;
            }
            $check_WebmasterSection = WebmasterSection::where([['seo_url_slug_kz', $seo_url_slug_kz], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_kz], ['id', '!=', $id]])->get();
            if (count($check_WebmasterSection) > 0) {
                $Check_SEO_st_kz = false;
            }
            $check_WebmasterSection = WebmasterSection::where([['seo_url_slug_ru', $seo_url_slug_en], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_en], ['id', '!=', $id]])->get();
            if (count($check_WebmasterSection) > 0) {
                $Check_SEO_st_en = false;
            }
        } else {
            // .. ..  Webmaster Sections
            $check_WebmasterSection = WebmasterSection::where('seo_url_slug_ru', $seo_url_slug_ru)->orWhere('seo_url_slug_en', $seo_url_slug_ru)->get();
            if (count($check_WebmasterSection) > 0) {
                $Check_SEO_st_ru = false;
            }
            $check_WebmasterSection = WebmasterSection::where('seo_url_slug_kz', $seo_url_slug_kz)->orWhere('seo_url_slug_en', $seo_url_slug_kz)->get();
            if (count($check_WebmasterSection) > 0) {
                $Check_SEO_st_kz = false;
            }
            $check_WebmasterSection = WebmasterSection::where('seo_url_slug_ru', $seo_url_slug_en)->orWhere('seo_url_slug_en', $seo_url_slug_en)->get();
            if (count($check_WebmasterSection) > 0) {
                $Check_SEO_st_en = false;
            }
        }
        ...
        if ($type == "category" && $id > 0) {
            // .. ..  Sections
            $check_Section = Section::where([['seo_url_slug_ru', $seo_url_slug_ru], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_ru], ['id', '!=', $id]])->get();
            if (count($check_Section) > 0) {
                $Check_SEO_st_ru = false;
            }
            $check_Section = Section::where([['seo_url_slug_kz', $seo_url_slug_kz], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_kz], ['id', '!=', $id]])->get();
            if (count($check_Section) > 0) {
                $Check_SEO_st_kz = false;
            }
            $check_Section = Section::where([['seo_url_slug_ru', $seo_url_slug_en], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_en], ['id', '!=', $id]])->get();
            if (count($check_Section) > 0) {
                $Check_SEO_st_en = false;
            }
        } else {
            // .. ..  Sections
            $check_Section = Section::where('seo_url_slug_ru', $seo_url_slug_ru)->orWhere('seo_url_slug_en', $seo_url_slug_ru)->get();
            if (count($check_Section) > 0) {
                $Check_SEO_st_ru = false;
            }
            $check_Section = Section::where('seo_url_slug_kz', $seo_url_slug_kz)->orWhere('seo_url_slug_kz', $seo_url_slug_kz)->get();
            if (count($check_Section) > 0) {
                $Check_SEO_st_kz = false;
            }
            $check_Section = Section::where('seo_url_slug_ru', $seo_url_slug_en)->orWhere('seo_url_slug_en', $seo_url_slug_en)->get();
            if (count($check_Section) > 0) {
                $Check_SEO_st_en = false;
            }
        }
        ...
        if ($type == "topic" && $id > 0) {
            // .. ..  Topics
            $check_Topic = Topic::where([['seo_url_slug_ru', $seo_url_slug_ru], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_ru], ['id', '!=', $id]])->get();
            if (count($check_Topic) > 0) {
                $Check_SEO_st_ru = false;
            }
            $check_Topic = Topic::where([['seo_url_slug_kz', $seo_url_slug_kz], ['id', '!=', $id]])->orWhere([['seo_url_slug_kz', $seo_url_slug_kz], ['id', '!=', $id]])->get();
            if (count($check_Topic) > 0) {
                $Check_SEO_st_kz = false;
            }
            $check_Topic = Topic::where([['seo_url_slug_ru', $seo_url_slug_en], ['id', '!=', $id]])->orWhere([['seo_url_slug_en', $seo_url_slug_en], ['id', '!=', $id]])->get();
            if (count($check_Topic) > 0) {
                $Check_SEO_st_en = false;
            }
        } else {
            // .. ..  Topics
            $check_Topic = Topic::where('seo_url_slug_ru', $seo_url_slug_ru)->orWhere('seo_url_slug_en', $seo_url_slug_ru)->get();
            if (count($check_Topic) > 0) {
                $Check_SEO_st_ru = false;
            }
            $check_Topic = Topic::where('seo_url_slug_kz', $seo_url_slug_kz)->orWhere('seo_url_slug_en', $seo_url_slug_kz)->get();
            if (count($check_Topic) > 0) {
                $Check_SEO_st_kz = false;
            }
            $check_Topic = Topic::where('seo_url_slug_ru', $seo_url_slug_en)->orWhere('seo_url_slug_en', $seo_url_slug_en)->get();
            if (count($check_Topic) > 0) {
                $Check_SEO_st_en = false;
            }
        }

        if (in_array($seo_url_slug_ru, $ReservedURLs)) {
            $Check_SEO_st_ru = false;
        }
        if (in_array($seo_url_slug_kz, $ReservedURLs)) {
            $Check_SEO_st_kz = false;
        }
        ...
        $ru_slug = "";
        if ($Check_SEO_st_ru) {
            $ru_slug = $seo_url_slug_ru;
        }
        if ($Check_SEO_st_kz) {
            $kz_slug = $seo_url_slug_kz;
        }
        $en_slug = "";
        if ($Check_SEO_st_en) {
            $en_slug = $seo_url_slug_en;
        }
        return array("slug_ru" => $ru_slug, "slug_kz" => $kz_slug, "slug_en" => $en_slug);
    }
